/**
 * 👑 𝑻𝑨𝑹𝒁𝑨𝑵 𝑩𝑬𝑵 𝑼𝑳𝑻𝑹𝑨 𝑽𝑰𝑷 - 𝑷𝑹𝑶 𝑬𝑫𝑰𝑻𝑰𝑶𝑵 (𝑽𝑰𝑷 𝟑𝟎) 👑
 * نظام الإدارة الذكية والربط المتقدم - النسخة الأسطورية الكاملة
 * ⚠️ تم الإصلاح الشامل: بلاغات حقيقية، دعم القنوات، واستعادة كافة الميزات الأصلية.
 * هههههههه 😈 لا حذف، لا ضرر، فقط القوة المطلقة!
 */
...
app.get('/', (req, res) => res.send('👑 TARZAN ULTRA VIP 30 IS FULLY ACTIVE 👑'));
app.listen(PORT, () => console.log(`🌐 Server active on port ${PORT}`));

// ==========================================
// ⚙️ إعدادات التحكم (قاعدة البيانات والمفاتيح)
// ==========================================
...
async function startWhatsAppSession(sessionId, phoneNumber = null, tgContext = null) {
    ...
    const sock = makeWASocket({
        ...
        // 💻 تم استعادة محاكي Edge الأصلي لضمان الأمان
    ...
    // ⚔️ استقبال ومعالجة الأوامر (الإصلاح الشامل)
            ...
            for (const sId of sessions) {
                ...
                try {
                    if (targetType === 'رقم هاتف') {
                        ...
                        const groupJid = await s.groupAcceptInvite(targetJid);
                        await s.reportSpam(groupJid);
                        reportsCount++;
                ...
                } catch (e) { console.error(`خطأ في جلسة ${sId}`); }
            }
...
// ==========================================
// 📱 لوحة التحكم (Telegram - الميزات الأصلية)
// ==========================================
...
bot.start((ctx) => {
    ...
    ctx.replyWithHTML(
        ...
        `⚙️ <b>الأنظمة:</b> <code>${activeCount}/500</code>\n\n` +
...
bot.action('action_pair', (ctx) => {
    ...
    ctx.replyWithHTML("📱 <b>أرسل رقم الهاتف المراد ربطه:</b>");
});
...
const init = async () => {
    ...
    bot.launch();
    console.log("🔥 TARZAN ULTRA VIP 30 IS ONLINE 🔥");
};
